...
- **TDD (Sección 8):** validación de esquema (columnas faltantes/tipos/integridad → fallo), adaptador S3/MinIO-parquet round-trip (`SparkSession` local + MinIO del K3s o `testcontainers-minio`), **adaptador JDBC con Testcontainers PostgreSQL** (levanta `<prefix>_readmodel` en test con datos de fixture), publicación de evento con embedded Kafka. Umbrales: validación/use cases ≥ 85%, adaptadores ≥ 80%.
...
**En `DEV-[proyecto]-05-tests.md`:** añadir el **E2E de reportería**: (a) camino feliz parquet `raw`→`processed`→3 formatos en `output/`; (b) validación fallida (columna faltante ⇒ `report.extraction.failed`, sin parquet). Ejecutado con MinIO + Kafka + OCI Functions (local) sobre el K3s del VPS.

...
- Añade el **Evento de Dominio** `ReportParquetGenerated` (MS→lambda de formatos), con payload: URL del `.parquet` en S3 y formato destino (csv/xls). Añade el evento de fallo `ReportETLFailed` (DE-xxx).
...
- `DS-xxx` — **Capa serverless de formatos**: una única Lambda Kafka Consumer que consume el evento `ReportParquetGenerated` publicado por el `report-etl-service`, genera el archivo en el formato pedido (csv/xls) a partir del `.parquet` en S3, y almacena el reporte final en S3. El formato se determina directamente del campo del evento; sin EventBridge intermedio.

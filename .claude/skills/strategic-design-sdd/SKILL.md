...
- Añade el **Evento de Dominio** `ReportParquetGenerated` (MS→función OpenFaaS de formatos), con payload: URL del `.parquet` en S3 y formato destino (csv/xls). Añade el evento de fallo `ReportETLFailed` (DE-xxx).
...
- `DS-xxx` — **ETL Spark unificado**: `report-etl-service` (un único job Spark que realiza extracción, validación de esquema y transformación por tipo de reporte con patrón Factory). Genera un `.parquet` almacenado en S3 y publica un evento Kafka (`ReportParquetGenerated`) con la URL del `.parquet` y el formato destino (csv/xls). Aclarar que **es un job batch ejecutado por schedule** (no un servicio REST persistente); no expone endpoints HTTP; el schedule define la frecuencia de ejecución (p. ej. diario, horario). Documenta el tradeoff: simplicidad operacional al eliminar el contrato inter-MS vs. latencia introducida por el schedule.
- `DS-xxx` — **Capa serverless de formatos**: una única función OpenFaaS (`report-format-consumer`, python3-http) invocada por el Kafka Connector de OpenFaaS cuando llega el evento `ReportParquetGenerated`; genera el archivo en el formato pedido (csv/xls) a partir del `.parquet` en MinIO/S3, y almacena el reporte final en almacenamiento de objetos. Desplegada via Helm (`openfaas/openfaas` + `openfaas/kafka-connector`) en K3s. El formato se determina directamente del campo del evento; sin EventBridge intermedio.
